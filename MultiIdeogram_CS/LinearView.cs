...
{
    public partial class LinearView : Form
    {
        ...
        public LinearView(string[] theFiles, string[] theFileNames, bool[] theAffecteds, string multiText)
        {
            InitializeComponent();
        ...
        private void LinearView_Load(object sender, EventArgs e)
        {
        ...
        public void GetData(bool ingnoreRSField, bool VCFGenotypes)
        {
            getRegions(ingnoreRSField, VCFGenotypes);
        ...
        private Dictionary<string, List<DNARegion>> getRegionsFromMultiTextFile()
        {
            ...
            try
            {
                ...
                while (fr.Peek() > 0)
                {
                    ...
                    else if (line.StartsWith("Common regions") == true)
                    {
                        fr.ReadToEnd();
                    }
                    else
        ...
        private Bitmap Draw(int multiple)
        {
            ...
            int bit = (SNPRegions.Length * two) - ten;
            if (bit < 0) { bit = 0; }
            int step = twentyfive + bit + four;
            int offset = (step * 22) + forty;

            int littledata = 0;
            if (SNPRegions.Length < 5)
            { littledata = two; }
            ...
            double widthScale = (Single)(247249719 / ((currentSize.Width - forty)));
            offset = 0;
            Font f = null;
            ...
            {
                int cl = Convert.ToInt32(chromosomelength[thisChromosome - 1] / widthScale);
                g.DrawString(thisChromosome.ToString(), f, Brushes.Black, two, offset + seventeen);
                DrawCytobandsImage(g, thisChromosome, 0, 247249719, twenty + offset, (float)widthScale, multiple);
                offset += step;
            }
            //Draw common regions
            ...
            if (MinimumSNPRegions != null)
            {
                foreach (DNARegion r in MinimumSNPRegions)
                {
                    if (r.Chromosome < 23)
                    {
                        int drawFrom = startHeight + ((r.Chromosome - 1) * step);
                        g.FillRectangle(colourCommon, (Single)(r.StartPoint / widthScale) + twenty, drawFrom + eighteen, (Single)(r.Length / widthScale), step + two - fifteen);
                    }
                }
                startHeight += one;
                offset += step;
            ...
            for (int i = 0; i < SNPRegions.Length; i++)
            {
                if (SNPRegions[i] != null)
                {
                    ...
                    {
                        if (count == 3 || littledata != 0)
                        {
                            for (int dottedline = 0; dottedline < 22; dottedline++)
                            {
                                int drawFrom = startHeight + ((dottedline) * step);
                    ...
                    startHeight += TheHeight + littledata;
                    offset += step;
            ...
            if (value <= currentSize.Width - twenty)
            {
                ...
                g.DrawString("0", f, Brushes.Black, value - four, offset + twentytwo);
            }
            for (int index = 10; index < increment + 1; index += 10)
            {
                value = (Single)(twenty + ((currentSize.Width - forty) * ((Single)index / increment)));
                if (value <= currentSize.Width - 20)
                {
                    g.DrawLine(Pens.Black, value, offset + fifteen, value, offset + eighteen);
                    g.DrawString(index.ToString(), f, Brushes.Black, value - four, offset + twentytwo);
                }
            }
            ...
            {
                int drawFrom = thirteen - four + ((Gene.Chromosome - 1) * step);
                g.FillRectangle(Brushes.Black, (Single)(Gene.StartPoint / widthScale) + twenty, twenty + drawFrom, two, step + four - fifteen);
            }
                ...
                for (int index = 0; index < FileNames.Length; index++)
                {
                    ...
                    if (SNPRegions[index] != null)
                    {
                        ...
                        else if (ExportValue == ExportThis.Long)
                        {
                            ...
                            System.IO.StreamWriter fw1 = new System.IO.StreamWriter(folder + "\\" + FileNames[index].Substring(0, FileNames[index].LastIndexOf(".")) + "_Multi-SNP.txt");
                            fw1.WriteLine("Chromosome\tStart\tEnd\tLength");
                            foreach (DNARegion line in SNPRegions[index])
                            { fw1.WriteLine(line.ToString()); }
                            fw1.Close();
        ...
        private void PlaceImage(Bitmap btm, bool resetScrollbar)
        {
            ...
            {
                Rectangle area;
                int h = btm.Height - 750 - vScrollBar1.Value;
                if (h < 0)
                { area = new Rectangle(0, vScrollBar1.Value + h, 750, 750); }
                else
                { area = new Rectangle(0, vScrollBar1.Value, 750, 750); }
        ...
        private void showDottedLineToolStripMenuItem_Click(object sender, EventArgs e)
        {
